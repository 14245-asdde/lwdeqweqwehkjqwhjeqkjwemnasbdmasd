...
export function Navbar({ onLogout }: NavbarProps) {
  ...
  const handleMarkRead = (notifId: string) => {
    if (user) { markNotificationRead(user.id, notifId); refreshUser(); }
  };
  const handleTeamInvite = (notifId: string, teamId: string, accept: boolean) => {
    if (user) { respondToTeamInvite(user.id, teamId, accept); markNotificationRead(user.id, notifId); refreshUser(); }
  };
  ...
  return (
    <nav className="navbar">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '60px' }}>
          ...
          <div className="hidden-mobile" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            {navItems.map(item => (
              ...
              >
                {item.page === 'admin' && <span style={{ color: '#00ff8c', marginRight: '4px' }}>⚡</span>}
                {item.label}
              </button>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <>
                ...
                <div style={{ position: 'relative' }}>
                  <button
                    ...
                    style={{ position: 'relative', background: 'rgba(124,58,255,0.08)', border: '1px solid rgba(124,58,255,0.2)', borderRadius: '8px', padding: '8px 10px', cursor: 'pointer', transition: 'all 0.2s' }}
                  >
                  ...
                  {notifOpen && (
                    <div className="notif-dropdown no-scrollbar" style={{ maxHeight: '380px', overflowY: 'auto' }}>
                      <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(124,58,255,0.12)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth="2.5">
                          <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/>
                        </svg>
                        <span className="font-orbitron" style={{ fontSize: '10px', letterSpacing: '0.1em', color: '#a855f7' }}>УВЕДОМЛЕНИЯ</span>
                      </div>
                      {user.notifications.length === 0 ? (
                        <div style={{ padding: '24px', textAlign: 'center', color: 'rgba(200,180,255,0.3)', fontSize: '13px' }}>Нет уведомлений</div>
                      ) : (
                        user.notifications.slice(0, 20).map(n => (
                          <div key={n.id} className={`notif-item ${!n.read ? 'unread' : ''}`}>
                            <p style={{ fontSize: '13px', color: 'rgba(200,180,255,0.7)', lineHeight: '1.4' }}>{n.message}</p>
                            <p style={{ fontSize: '10px', color: 'rgba(200,180,255,0.25)', marginTop: '4px', fontFamily: 'Share Tech Mono, monospace' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', background: 'rgba(8,6,20,0.6)', border: '1px solid rgba(120,60,255,0.15)', borderRadius: '8px' }}>
                  {user.isAdmin && <span style={{ color: '#00ff8c', fontSize: '10px' }}>⚡</span>}
                  <span className="font-mono-tech" style={{ fontSize: '13px', color: 'rgba(200,180,255,0.6)' }}>{user.username}</span>
                </div>
            ...
            <button
              ...
              style={{ display: 'none', background: 'rgba(124,58,255,0.08)', border: '1px solid rgba(124,58,255,0.2)', borderRadius: '8px', padding: '8px', cursor: 'pointer' }}
              className="mobile-btn"
            >
              {mobileOpen
                ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth="2.5"><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="18" x2="21" y2="18"/></svg>
              }
            </button>
          </div>
        </div>
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="mobile-menu-anim" style={{ background: 'rgba(5,5,12,0.98)', borderTop: '1px solid rgba(124,58,255,0.12)', padding: '12px 20px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
          {navItems.map(item => (
            <button
              key={item.page}
